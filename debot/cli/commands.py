...
from debot import __logo__, __version__

app = typer.Typer(
    name="debot",
    help=f"{__logo__} debot - Personal AI Assistant",
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} debot v{__version__}")
        raise typer.Exit()
...
@app.command()
def onboard():
    """Initialize debot configuration and workspace."""
    from debot.config.loader import get_config_path, save_config
    from debot.config.schema import Config
    from debot.utils.helpers import get_workspace_path
...
I am debot, a lightweight AI assistant.
...
def config_compaction(
    ...
    from debot.config.loader import get_config_path, load_config, save_config
...
def config_compaction_model(
    ...
    from debot.config.loader import get_config_path, load_config, save_config
    from debot.config.schema import CompactionOverride
...
def gateway(
    ...
    from debot.bus import MessageBus
    from debot.channels.manager import ChannelManager
    from debot.config.loader import get_data_dir, load_config
    from debot.cron import CronJob, CronService
    from debot.heartbeat import HeartbeatService
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        if job.payload.deliver and job.payload.to:
            from debot.bus import OutboundMessage
...
def _get_bridge_dir() -> Path:
    ...
    user_bridge = Path.home() / ".debot" / "bridge"
    ...
    pkg_bridge = Path(__file__).parent / "bridge"  # debot/bridge (installed)
    ...
    if not source:
        ...
        console.print("Try reinstalling: pip install --force-reinstall debot")
        raise typer.Exit(1)
...
def skills_install(
    ...
    Examples:
      debot skills install find-skills
      debot skills install file:///path/to/my.skill
    """
    from debot.utils.helpers import get_workspace_path
    ...
    if name_or_url.startswith(("http://", "https://", "file://")):
        ...
        try:
            from debot.skills.installer import install_from_url
...
def skills_list(
    ...
    try:
        ...
        from debot.skills.find_skills.run import list_skills
...
def cron_add(
    ...
    """Add a scheduled job."""
    from debot.config.loader import get_data_dir
    from debot.cron import CronSchedule, CronService
